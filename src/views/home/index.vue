...
export default {
  ...
  methods: {
    setCurveChartFn () {
      // 获取你想放进去的容器
      const curveChart = echarts.init(document.getElementById('curve_show'))// 初始化echart，页面要挂载好了才能使用
      const arrylist = [
        {
          count: 36,
          date: '2019-04-13'
      ...
      for (let i = 0; i < arrylist.length; i++) {
        aCount.push(arrylist[i].count)
        aDate.push(arrylist[i].date)
      }
      const chartopt = {
        title: {
          text: '月新增文章数', // 标题
  ...
  mounted () {
    ...
    this.picChartFn()// 设置扇形
    this.columnChartFn()// 设置柱状图
  }
}
</script>
